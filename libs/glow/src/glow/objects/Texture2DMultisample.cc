...
SharedTexture2DMultisample Texture2DMultisample::create(size_t width, size_t height, GLenum internalFormat)
{
...
void Texture2DMultisample::BoundTexture2DMultisample::setMinFilter(GLenum filter)
{
    ...
    if (corrected)
        warning() << "Texture2DMultisample does not support MipMapping.";
...
void Texture2DMultisample::BoundTexture2DMultisample::resize(size_t width, size_t height)
{
    if (!isCurrent())
        return;
    checkValidGLOW();
