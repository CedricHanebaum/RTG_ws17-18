...
SharedTexture2D Texture2D::create(size_t width, size_t height, GLenum internalFormat)
{
...
SharedTexture2D Texture2D::createStorageImmutable(size_t width, size_t height, GLenum internalFormat, int mipmapLevels)
{
    auto tex = std::make_shared<Texture2D>(internalFormat);
...
void Texture2D::BoundTexture2D::makeStorageImmutable(size_t width, size_t height, GLenum internalFormat, int mipmapLevels)
{
    if (!isCurrent())
        return;
...
void Texture2D::BoundTexture2D::resize(size_t width, size_t height)
{
    if (!isCurrent())
        return;
    checkValidGLOW();
...
void Texture2D::BoundTexture2D::setSubData(size_t x, size_t y, size_t width, size_t height, GLenum format, GLenum type, const GLvoid *data, int mipmapLevel)
{
    if (!isCurrent())
        return;
    checkValidGLOW();
