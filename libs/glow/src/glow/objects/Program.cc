...
bool Program::linkAndCheckErrors()
{
    ...
    if (logLength > 1)
    {
        ...
        error() << "Linker log for:";
        for (auto const &s : mShader)
            error() << "  " << glShaderTypeToString(s->getType()) << ": "
                    << (s->getFileName().empty() ? "[Non-file shader]" : s->getFileName());
...
GLint Program::getUniformLocation(const std::string &name) const
{
    ...
    checkValidGLOW();
    GLint loc = glGetUniformLocation(mObjectName, name.c_str());
    mUniformCache.push_back(std::make_pair(name, loc));
    return loc;
}
...
void Program::link(bool saveUniformState)
{
    ...
    while (requiresRelink)
    {
        ...
        if (linkCnt > 2)
        {
            warning() << "Linking does not converge. Aborting.";
            break;
        }
    }
    ...
    if (!mCheckedForAttributeLocationLayout)
    {
        ...
        for (auto const &mapping : mAttributeMapping->getMap())
        {
            ...
            if (shaderLoc != mapping.second)
            {
                error() << "Attribute '" << mapping.first << "' has inconsistent location (" << mapping.second << " vs "
                        << shaderLoc << "). Note, use of layout(location = X) is not allowed!";
            }
        }
        ...
        for (auto const &mapping : mAttributeMapping->getMap())
        {
            ...
            {
                error() << "Attribute '" << mapping.first << "' has inconsistent location (" << ((mapping.second + 1) % maxAttrs)
                        << " vs " << shaderLoc << "). Note, use of layout(location = X) is not allowed!";
            }
        }
    ...
    if (!mCheckedForFragmentLocationLayout)
    {
        ...
        for (auto const &mapping : mFragmentMapping->getMap())
        {
            ...
            if (shaderLoc != mapping.second)
            {
                error() << "Fragment output '" << mapping.first << "' has inconsistent location (" << mapping.second
                        << " vs " << shaderLoc << "). Note, use of layout(location = X) is not allowed!";
            }
        }
        ...
        for (auto const &mapping : mFragmentMapping->getMap())
        {
            ...
            if (shaderLoc != (mapping.second + 1) % maxFrags)
            {
                error() << "Fragment output '" << mapping.first << "' has inconsistent location ("
                        ...
                        << "). Note, use of layout(location = X) is not allowed!";
            }
        }
...
void Program::configureTransformFeedback(const std::vector<std::string> &varyings, GLenum bufferMode)
{
    ...
    {
        glow::error() << "Cannot set configureTransformFeedback while same program is active.";
        assert(0);
        return;
    }
...
SharedProgram Program::createFromFiles(const std::vector<std::string> &filenames)
{
    ...
    for (auto const &filename : filenames)
        if (Shader::resolveFile(filename, type, content, realFileName))
        {
            ...
            error() << "Unable to resolve shader file '" << filename << "'.";
...
{
    GLOW_RUNTIME_ASSERT(sCurrentProgram == this, "Currently bound Program does NOT match represented program", return false);
    return true;
}
