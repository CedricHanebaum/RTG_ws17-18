...
private:
    ...
    mutable std::vector<std::pair<std::string, GLint>> mUniformCache;
    ...
    std::map<size_t, SharedAtomicCounterBuffer> mAtomicCounterBuffers;
...
public:
    ...
    public: // gl functions with use
        ...
        void setImage(size_t bindingLocation, SharedTexture const& tex, GLenum usage = GL_READ_WRITE, int mipmapLevel = 0, int layer = 0);
...
public:
    ...
    /// DOES NOT REQUIRE PROGRAM USE
    void setAtomicCounterBuffer(size_t bindingPoint, SharedAtomicCounterBuffer const& buffer);
