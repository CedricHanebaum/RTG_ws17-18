...
static void attachToFramebuffer(SharedTexture const &tex, GLenum attachment, int mipmapLevel, int layer)
{
    ...
    default:
        error() << "Unsupported texture target " << tex->getTarget();
        break;
    }
}
...
{
    GLOW_RUNTIME_ASSERT(sCurrentBuffer == this, "Currently bound FBO does NOT match represented buffer", return false);
    return true;
}
...
void Framebuffer::BoundFramebuffer::attachDepth(GLenum depthFormat)
{
    ...
    GLOW_RUNTIME_ASSERT(buffer->mColorAttachments.size() > 0, "Requires already attached color textures!", return );
    checkValidGLOW();
...
Framebuffer::BoundFramebuffer::BoundFramebuffer(Framebuffer *buffer) : buffer(buffer)
{
    ...
    if (readFBO != previousBuffer)
        error() << "GLOW does not support differing READ and WRITE Framebuffer.";
