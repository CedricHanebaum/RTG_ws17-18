...
public:
    ...
    struct BoundTextureCubeMap
    {
        ...
        void setData(GLenum internalFormat, GLenum target, size_t width, size_t height, GLenum format, GLenum type, const GLvoid* data, int mipmapLevel = 0);
        /// Data upload via glm or c++ type (see gltypeinfo)
        template <typename DataT>
        void setData(GLenum internalFormat, GLenum target, size_t width, size_t height, std::vector<DataT> const& data, int mipmapLevel = 0)
        {
        ...
        void setData(GLenum internalFormat, GLenum target, size_t width, size_t height, const DataT(&data)[N], int mipmapLevel = 0)
        {
            if (N != width * height)
            {
        ...
        void setSubData(GLenum target, size_t x, size_t y, size_t width, size_t height, GLenum format, GLenum type, const GLvoid* data, int mipmapLevel = 0);
        /// Partial data upload via glm or c++ type (see gltypeinfo)
        template <typename DataT>
        void setSubData(GLenum target, size_t x, size_t y, size_t width, size_t height, std::vector<DataT> const& data, int mipmapLevel = 0)
        {
            if (data.size() != width * height)
            {
                error() << "Texture size is " << width << " x " << height << " = " << width * height << " but " << data.size()
        ...
        template <typename DataT, std::size_t N>
        void setSubData(GLenum target, size_t x, size_t y, size_t width, size_t height, const DataT(&data)[N], int mipmapLevel = 0)
        {
            if (N != width * height)
            {
                error() << "Texture size is " << width << " x " << height << " = " << width * height << " but " << N
        ...
        template <typename DataT, size_t width, size_t height>
        void setSubData(GLenum target, size_t x, size_t y, const DataT(&data)[height][width] , int mipmapLevel = 0)
        {
            setSubData(target, x, y, width, height, glTypeOf<DataT>::format, glTypeOf<DataT>::type, data, mipmapLevel);
        }
...
public: // static construction
    ...
    static SharedTextureCubeMap create(size_t width = 1, size_t height = 1, GLenum internalFormat = GL_RGBA);
    /// Creates a CubeMap texture with given width and height which is storage immutable
    /// If mipmapLevels is <= 0, log2(max(width, height)) + 1 is used
    static SharedTextureCubeMap createStorageImmutable(size_t width, size_t height, GLenum internalFormat, int mipmapLevels = 0);

    /// Creates a CubeMap texture from file
    /// See TextureData::createFromFile for format documentation
    /// Uses preferred internal format
    static SharedTextureCubeMap createFromFile(std::string const& filename, ColorSpace colorSpace = ColorSpace::AutoDetect);
    /// same as createFromFile but with custom internal format
    static SharedTextureCubeMap createFromFile(std::string const& filename, GLenum internalFormat, ColorSpace colorSpace = ColorSpace::AutoDetect);
