...
void Assignment10::update(float elapsedSeconds)
{
...
void Assignment10::render(float elapsedSeconds)
{
...
void Assignment10::renderShadowPass()
{
...
void Assignment10::renderDepthPrePass()
{
...
void Assignment10::renderOpaquePass()
{
...
void Assignment10::renderLightPass()
{
...
void Assignment10::renderSSRPass()
{
    if (!mShowSSR)
    {
...
void Assignment10::renderTransparentPass()
{
    // debug: wireframe rendering
...
void Assignment10::renderTransparentResolve()
{
...
void Assignment10::renderHDRPass()
{
    GLOW_SCOPED(disable, GL_DEPTH_TEST);
    GLOW_SCOPED(disable, GL_CULL_FACE);
...
void Assignment10::renderOutputStage()
{
    GLOW_SCOPED(disable, GL_DEPTH_TEST);
    GLOW_SCOPED(disable, GL_CULL_FACE);
...
void Assignment10::renderScene(camera::CameraBase* cam, RenderPass pass)
{
    // set up general purpose shaders
    switch (pass)
    {
    case RenderPass::Opaque:
        setUpShader(mShaderPlants.get(), cam, pass);
        break;
    ...
    // render terrain
    {
        ...
        // render jobs
        std::vector<TerrainJob> jobsTerrain;
        std::vector<PlantJob> jobsPlants;
        ...
        // collect meshes per material and shader
        {
            for (auto const& chunkPair : mWorld.chunks)
            {
                ...
                for (auto const& mesh : chunk->queryMeshes())
                {
                    ...
                    // Vegetation (BEFORE custom BFC)
                    if (pass == RenderPass::Opaque && mesh.abPlants->getElementCount() > 0)
                        jobsPlants.push_back({mesh.vaoPlants.get(), camDis});
                    ...
                    // add render job
                    jobsTerrain.push_back({shader, mat, vao, camDis});
                }
            }
        }
        ...
        // .. sort renderjobs
        {
            ...
            // .. material
            // .. front-to-back
            std::sort(jobsTerrain.begin(), jobsTerrain.end(), [](TerrainJob const& a, TerrainJob const& b) {
        ...
        {
            auto idxShader = 0u;
            while (idxShader < jobsTerrain.size())
            {
                // set up shader
                auto program = jobsTerrain[idxShader].program;
                setUpShader(program, cam, pass);
                auto shader = program->use();

                // .. per material
                auto idxMaterial = idxShader;
                while (idxMaterial < jobsTerrain.size() && jobsTerrain[idxMaterial].program == program)
                {
                    auto mat = jobsTerrain[idxMaterial].mat;
                    ...
                    while (idxMesh < jobsTerrain.size() && jobsTerrain[idxMesh].mat == mat)
                    {
                        auto mesh = jobsTerrain[idxMesh].mesh;
        ...
        if (!jobsPlants.empty())
        {
            ...
            auto shader = mShaderPlants->use();

            // texture
            shader.setTexture("uTexPlants", mTexPlants);

            for (auto const& job : jobsPlants)
                job.mesh->bind().draw(); // with instancing!
        }
    }
...
void Assignment10::swapBloomTargets()
{
    std::swap(mTexBloomDownsampledA, mTexBloomDownsampledB);
    std::swap(mFramebufferBloomToA, mFramebufferBloomToB);
}

void Assignment10::setUpShader(glow::Program* program, camera::CameraBase* cam, RenderPass pass)
{
    auto shader = program->use();
...
void Assignment10::setUpLightShader(glow::Program* program, glow::camera::CameraBase* cam)
{
    auto shader = program->use();
...
void Assignment10::buildLineMesh()
{
...
void Assignment10::drawLine(glm::vec3 from, glm::vec3 to, glm::vec3 color, RenderPass pass)
{
    if (pass != RenderPass::Transparent)
    {
...
void Assignment10::drawAABB(glm::vec3 min, glm::vec3 max, glm::vec3 color, RenderPass pass)
{
    if (pass != RenderPass::Transparent)
    {
        glow::error() << "not implemented.";
...
void Assignment10::spawnLightSource(glm::vec3 const& origin)
{
    LightSource ls;
    ls.position = origin;
...
void Assignment10::updateLightSources(float elapsedSeconds)
{
...
void Assignment10::init()
{
    ...
    // load shaders
    {
        ...
        mShaderPlants = Program::createFromFile(shaderPath + "objects/plants");
    ...
    // load textures
    {
        ...
        mTexPlants = Texture2D::createFromFile(texPath + "plants.png");
        mTexPlants->bind().setWrap(GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
...
void Assignment10::getMouseRay(glm::vec3& pos, glm::vec3& dir) const
{
...
void Assignment10::updateViewRay()
{
    // calculate mouse ray
    glm::vec3 pos, dir;
    getMouseRay(pos, dir);
...
void Assignment10::createTerrainShader(std::string const& name)
{
...
void Assignment10::updateShadowMapTexture()
{
...
bool Assignment10::onMouseButton(double x, double y, int button, int action, int mods, int clickCount)
{
...
bool Assignment10::onMousePosition(double x, double y)
{
...
bool Assignment10::onKey(int key, int scancode, int action, int mods)
{
...
void Assignment10::onResize(int w, int h)
{
...
void Assignment10::setUpTweakBar()
{
