                  ...
                  std::vector<TerrainMeshData> &newMeshes)
{
    ...
    for (auto pdir = 0; pdir < 6; ++pdir)
    {
        ...
        {
            mesh.aabbMin = min(pos, mesh.aabbMin);
            mesh.aabbMax = max(pos, mesh.aabbMax);
        }
...
std::vector<TerrainMeshData> generateMesh(const std::vector<Block> &blocks, glm::ivec3 chunkPos)
{
