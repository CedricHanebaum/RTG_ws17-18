...
float shadowing(vec3 worldPos, vec3 L)
{
    vec3 shadowDir = L;
    vec3 offset = shadowDir * 0.15; // to prevent shadow acne
...
//  - skybox reflection
//  - shadowing
vec3 shadingComplete(vec3 worldPos, vec3 N, vec3 V, vec3 L, float AO, float roughness, vec3 diffuse, vec3 specular, float reflectivity)
{
    vec3 color = vec3(0.0);
    ...
    {
        // ambient
        color += uAmbientLight * AO * diffuse;
        ...
        color += uLightColor * mix(cDiffuse, cSpecular, F) * shadowing(worldPos, L);
